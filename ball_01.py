...
# 4. 예측 경로를 빨간색 곡선으로 표현(1초 후)
# 5. 영상 저장
# ------------------------------------------------------------------------------------------------------------
#
# --------------------------------------------------------------
# UKF
# 매 프레임마다 예측 - 업데이트 두가지 과정을 거침
#
# 예측 단계 ukf.predict() 
# 이전 상태 벡터 x(위치, 속도, 가속도)를 가지고 다음 상태를 예측
# 시그마 포인트{(2 * 차원의 수) + 1 = 13개}라는 여러 가상 샘플을 만듬
# 시그마 포인트를 등가속도 운동 방정식을 기반으로 한 예측 함수 fx()에 통과시킴
# 나온 평균 및 분산을 계산하여 상태를 예측
#
# 업데이트 단계 ukf.update()
# 카메라에서 실제 측정된 탁구공의 위치를 이용해 예측된 상태를 보정
# 측정값은 hx()를 통해 얻어짐
# 예측된 위치와 실제 측정된 위치의 차이를 이용해 상태를 조정
# --------------------------------------------------------------
...
while True: 
    ...
    ### 탁구공 예측 궤적 설정 ###
    future_steps = int(1.0 / dt)                                            # dt(30bps)를 사용, 1초 후를 예측하기 위해 future_steps(15 프레임)을 선언
